...
public class KekeFallEvent {
    ...
    public void onFall(LivingFallEvent livingFallEvent) {
        ...
        if (!entity.level.isClientSide()) {
            //Ice
            if (entity.fallDistance >= ConfigurableFallsCommonConfigs.ICE_BREAK_ON_FALL_FALL_DISTANCE.get() && ConfigurableFallsCommonConfigs.ICE_BREAK_ON_FALL.get() && getBlockStateOn.getBlock() == Blocks.ICE) {
                entity.level.destroyBlock(blockPos, true, entity);
            }
            //Glass
            if (entity.fallDistance >= ConfigurableFallsCommonConfigs.GLASS_BREAK_ON_FALL_FALL_DISTANCE.get() && ConfigurableFallsCommonConfigs.GLASS_BREAK_ON_FALL.get() && getBlockStateOn.getBlock() instanceof AbstractGlassBlock) {
                entity.level.destroyBlock(blockPos, ConfigurableFallsCommonConfigs.GLASS_BREAK_ON_FALL_DROPS.get(), entity);
            }
            if (entity.fallDistance >= ConfigurableFallsCommonConfigs.GLASS_PANE_BREAK_ON_FALL_FALL_DISTANCE.get() && ConfigurableFallsCommonConfigs.GLASS_PANE_BREAK_ON_FALL.get() && getBlockStateOn.getBlock() == Blocks.GLASS_PANE) {
                entity.level.destroyBlock(blockPos, false, entity);
            }
            if (entity.fallDistance >= ConfigurableFallsCommonConfigs.GLASS_PANE_BREAK_ON_FALL_FALL_DISTANCE.get() && ConfigurableFallsCommonConfigs.GLASS_PANE_BREAK_ON_FALL.get() && getBlockStateOn.getBlock() instanceof StainedGlassPaneBlock) {
                entity.level.destroyBlock(blockPos, false, entity);
            }
            //Snow
